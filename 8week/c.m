...
%This is a specifice example for 6.1-c
...
a0 = int(x(t),t,tStart,tStop)/To %apply the formula of A0 to the inputed values
aSym(n)  = simplify(int(x(t)*cos(n*wo*t),t,tStart,tStop)/(To/2))%the formula for an
bSym(n)  = simplify(int(x(t)*sin(n*wo*t),t,tStart,tStop)/(To/2))%the formula for bn
